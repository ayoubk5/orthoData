...
let DESKTOP_PATH = 'c:/Users/akermaoui/Downloads/orthodata/temp_desktop';

console.log("DEBUG: DESKTOP_PATH is:", DESKTOP_PATH);
if (!DESKTOP_PATH) {
    console.error("\n=======================================================");
...
app.post('/api/open-folder', authenticateToken, (req, res) => {
    try {
        ...
        console.log("📂 [DEBUG] Demande d'ouverture pour :", folderName);
        if (!folderName) {
            return res.status(400).json({ success: false, error: "Nom de dossier manquant" });
        }

        const folderPath = path.join(PATIENTS_FOLDER, folderName);
        const normalizedPath = path.normalize(folderPath);

        console.log("🔍 [DEBUG] Chemin complet :", normalizedPath);

        if (!fs.existsSync(folderPath)) {
            console.error("❌ [DEBUG] Le dossier n'existe pas sur le disque.");
            return res.status(404).json({ success: false, error: "Dossier introuvable sur le disque" });
        }

        const command = `start "" "${folderPath}"`;
        console.log("💻 [DEBUG] Exécution commande :", command);

        exec(command, { windowsHide: true }, (error, stdout, stderr) => {
            if (error) {
                console.error("❌ [DEBUG] Erreur exec:", error);
                return res.status(500).json({ success: false, error: "Erreur Windows: Impossible d'ouvrir le dossier" });
            }
            console.log("✅ [DEBUG] Dossier ouvert avec succès !");
            res.json({ success: true, message: "Dossier ouvert" });
        });

    } catch (e) {
        console.error("❌ [DEBUG] Erreur Catch:", e);
